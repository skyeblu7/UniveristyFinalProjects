...
void forward(mshadow::Tensor<cpu, 4, DType> &y, const mshadow::Tensor<cpu, 4, DType> &x, const mshadow::Tensor<cpu, 4, DType> &k)
{
    ...
    */
    ...
    for (int b = 0; b < B; ++b)                     // iterate over each image
        for (int m = 0; m < M; ++m)                 // iterate over feature maps
            for (int h = 0; h < H_out; ++h)             // compute each output
                for (int w = 0; w < W_out; ++w) {
                    y[b][m][h][w] = 0;
                    for (int c = 0; c < C; ++c)     // sum the input feature maps
